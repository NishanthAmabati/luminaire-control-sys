...
/**
 * Independent Charts component that manages its own WebSocket subscription
 * Updates charts in real-time without relying on parent state
 */
const IndependentCharts = ({ ws, isLoading, theme, isSystemOn, autoMode, sceneData, verticalLinePosition }) => {
  // Subscribe to current values independently
  const currentValues = useCurrentValues(ws);

  const chartData = useMemo(() => {
    const centerPosition = autoMode ? verticalLinePosition : 4320
    ...
    return {
      datasets: [
        ...
        ...(isSystemOn ? [{
          label: "current CCT",
          data: currentValues.current_cct ? (autoMode ? [{ x: centerPosition, y: currentValues.current_cct }] : [{ x: 0, y: currentValues.current_cct }, { x: 8640, y: currentValues.current_cct }]) : [],
          borderColor: annotationColor,
          backgroundColor: annotationColor,
          pointStyle: autoMode ? "circle" : false,
          pointRadius: autoMode ? 7 : 0,
          pointHoverRadius: autoMode ? 9 : 0,
          showLine: !autoMode,
          borderWidth: autoMode ? 0 : 2,
          tension: 0,
        }] : []),
      ],
    }
  }, [sceneData.cct, isSystemOn, currentValues.current_cct, theme, verticalLinePosition, autoMode])
    ...
    return {
      datasets: [
        ...
        ...(isSystemOn ? [{
          ...
          data: currentValues.current_intensity ? (autoMode ? [{ x: centerPosition, y: currentValues.current_intensity }] : [{ x: 0, y: currentValues.current_intensity }, { x: 8640, y: currentValues.current_intensity }]) : [],
          borderColor: annotationColor,
          backgroundColor: annotationColor,
          pointStyle: autoMode ? "circle" : false,
          pointRadius: autoMode ? 7 : 0,
  ...
  }, [sceneData.intensity, isSystemOn, currentValues.current_intensity, theme, verticalLinePosition, autoMode])
  ...
  const chartOptions = useMemo(
    () => ({
      ...
      plugins: {
        ...
        currentValueLabel: {
          text: `Current CCT: ${currentValues.current_cct.toFixed(1)}K`,
        },
        title: {
          display: true,
          text: "CCT PROFILE",
    ...
    [theme, verticalLinePosition, currentValues.current_cct, autoMode, isSystemOn]
  )
  ...
  const intensityChartOptions = useMemo(
    () => ({
      ...
      plugins: {
        plotAreaBackground: plotAreaBackgroundPlugin,
        currentValueLabel: {
          text: `Current Intensity: ${currentValues.current_intensity.toFixed(1)} lux`,
        },
        title: {
          display: true,
    ...
    [theme, verticalLinePosition, currentValues.current_intensity, autoMode, isSystemOn]
  )
...
export default IndependentCharts
