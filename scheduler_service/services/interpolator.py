...
class InterpolationStrategy(ABC):
    ...
    def _handle_initial_point(self, scene, now_sec, runtime):
        if not scene or not scene[0].get("initial"):
            return False
        ...
        if first_real:
            ...
            if now_sec < t_first:
                runtime.cct = scene[0]["cct"]
                runtime.lux = scene[0]["lux"]
                runtime.progress = 0.0
                log.debug("interpolation_initial_hold", cct=runtime.cct, lux=runtime.lux)
                return True
        return False
...
class LinearInterpolation(InterpolationStrategy):
    def compute(self, scene, now_sec, runtime):
        if self._handle_initial_point(scene, now_sec, runtime):
            return
