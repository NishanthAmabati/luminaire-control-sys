...
#
# Multiple services can be specified as comma-separated:
#   "web,gateway"         -> webapp + event-gateway
#   "state,scheduler"    -> state-service + scheduler-service
#   "python,web"          -> all Python services + webapp
...
VALID_SERVICES="all,web,gateway,python,state,scheduler,timer,metrics,luminaire"

validate_service() {
    local svc="$1"
    local found=false
    for valid in $(echo "$VALID_SERVICES" | tr ',' ' '); do
        if [[ "$svc" == "$valid" ]]; then
            found=true
            break
        fi
    ...
    if ! $found; then
        echo "ERROR: Invalid service '$svc'. Valid services are: $VALID_SERVICES" >&2
        exit 1
    fi
}
...
# Split comma-separated services and process each
IFS=',' read -ra SERVICE_ARRAY <<< "$SERVICE_SELECTION"
